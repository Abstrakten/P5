...
\begin{tikzpicture}
    ...
    \addplot[blue,line width=1pt] 
         table [x=ms, y=pos_actual, col sep=comma] {sections/data/kalman_data_42.csv};
         \addlegendentry{Measured position}
    \addplot[red,line width=1pt] 
         table [x=ms, y=pos_pred, col sep=comma] {sections/data/kalman_data_42.csv};
         \addlegendentry{Predicted position}
\end{axis}
\end{tikzpicture}
\caption{Kalman results with initial position at -42.}\label{kalman42}
\end{figure}
\FloatBarrier